...
/**
 * Contributions:
 * Orange Labs: added SECURE PROXY features, all modifications are prefixed by the tag [SECURE_PROXY]
 * 
 */
...
function confirm_userinfo(){
  ...
  log_info('requested claims = %s', print_r($requested_claims, true));
    ...
    // [SECURE_PROXY] - display client application name
    $client_html = "";
	
    $client = db_get_client($req['client_id']);
    
    // check if secure_proxy is present
    if (isset ( $req ['secure_proxy'] )) {
		if (isset ( $req ['secure_proxy'] ['app_id'] ) && isset ( $req ['secure_proxy'] ['app_id'] ['value'] )) {
		    // [SECURE_PROXY] - display secure_proxy name and client application name
			$sp_app_id = $req ['secure_proxy'] ['app_id'] ['value'];
			$client_sp = db_get_client ( $sp_app_id );
			$client_html = '<thead><tr><td><b>Via secure proxy<b></td><td>'.$client['client_name'].'</td></tr>
					<tr><td>For application</td><td>'.$client_sp['client_name'].'</td></tr>';
		}
	} else {
	    // [SECURE_PROXY] - display client application name only
...
function handle_auth() {
    ...
    try{
        ...
        // [SECURE_PROXY] save parameters in session
...
//                      throw new OidcException('invalid_request', 'Request Object missing required parameters');
                ...
                // [SECURE_PROXY] check if secure proxy request
                if( isset( $_SESSION['rpfA']['secure_proxy'] ) ) {
                	$sec_proxy = $_SESSION['rpfA']['secure_proxy'];
                	if( isset( $sec_proxy['app_id'] ) && isset( $sec_proxy['app_id']['value']) ) {
                		$sp_app_id = $sec_proxy['app_id']['value'];
                		if( !db_get_client($sp_app_id) ) {
                        	throw new OidcException('invalid_request', "app id '$sp_app_id' not found");
                		}
                	}
                
                log_debug("handle_auth $sec_proxy for application : %s",$sp_app_id);
                } 
            }
        } else {
		...
		if ($grant_type == 'refresh_token') {
			...
			if ($client_authenticated) {
				...
				if (in_array ( 'openid', $scopes )) {
					...
					// [SECURE_PROXY] add the original application id in the audience of the id_token and add the azp claim
					$original_application = $request_info ['r'] ['sec_proxy'] ['app_id'] ['value'];
				...
				$json_response = json_encode ( $token_response );
				if( $cryptJson == true ) {
					if($encryption_keys!=null) {
						$alg = 'RSA1_5';
						$enc = 'A128CBC-HS256'; 
						$json_response = jwt_encrypt($json_response, $encryption_keys[0], false, NULL, $jwk_uri, NULL, $alg, $enc, false);
					} else {
						send_error ( NULL, 'invalid_client', 'invalid client jwk' );
					}
				}
				echo $json_response;
			} else
			...
			if ($client_authenticated) {
				...
				// [SECURE_PROXY] fix time : using timestamp
				$issue_at = time ();
				$expiration_at = time () + (30 * 60);
				$fields = array (
				...
				if (in_array ( 'offline_access', $scopes ) && in_array ( 'code', $response_types ) && in_array ( 'token', $response_types ) && in_array ( 'consent', $prompts )) {
					...
					// [SECURE_PROXY] : fix times : using timestamp
...
// [SECURE_PROXY] adding the tim_app_key parameter to the function in order to be able to create id token with them
